...
class DateTimePicker extends Events {
  ...
  delegateEvents() {
    ...
    this.$('.js-date-am-pm-mode').addEventListener('click', e => this.showHourClock(e), false);
  ...
  setTime(time) {
    const m = moment(time).clone();
    // round minutes to the nearest 5 minute mark
    m.minutes(Math.round(m.minutes() / 5) * 5);
    const minuteAsInt = m.minutes();
    m.minutes(minuteAsInt);

    const hour = m.format(this.options.twentyFourHours ? 'HH' : 'hh');
    const minutes = m.format('mm');
    const hourAsInt = this.options.twentyFourHours
      ? parseInt(hour, 10) % 24
      : parseInt(hour, 10) % 12;
